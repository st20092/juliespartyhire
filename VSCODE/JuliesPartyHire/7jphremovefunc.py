...
#Submit Button
submit = Button(registry, text="Submit", command=validate)
...
customertable.insert(parent="", index="end", iid=3, values=("Benedict", "Jed", "1", "Balloons", "43"))
...
#Remove Frame
removesection = LabelFrame(database)
removesection.grid(row=1, column=0)
#Removing Buttons
deleteall = Button(removesection, text="Remove All Data", command=removeall)
deleteall.grid(row=0, column=0)

deleteselected = Button(removesection, text="Remove Selected", command=removeselected)
deleteselected.grid(row=0, column=1)
